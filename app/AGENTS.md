...
- Repeat the same editable email as `CONTACT · address` immediately to the right of the public `LinkSee` brand. On desktop and tablet it shares the header row with the event name; below 700px the brand and full email stay on the first row while the event name moves to a second row aligned left beneath the brand.
...
- Do not render `01–04` number rails beside the four main content-section headings. Their eyebrow, title, and description align directly to the left content edge. In the project audience card, group the label and body at the top with a `28px` gap; retain the standard card bottom inset and the original `76px` gap before the feature grid.
- Prefix the four main content titles inline with `一、` through `四、`; the fourth title and eyebrow are `四、参赛目标` and `OUR GOALS / 参赛目标`. Keep the Hero project title and editable project data unnumbered.
...
- Keep the public footer free of admin-entry links, and do not render the internal research-note copy on the public project page. The admin route remains available directly at `/admin`.
- Render the six-step journey as a single horizontal editorial route on desktop, a 3 × 2 card route on tablet, and a vertical timeline on mobile. In `/admin`, show the six steps as clearly numbered editor cards.
- Hosted team content is authoritative in D1 through `/api/team`; the public page loads that API first and falls back to the bundled JSON only when the API is unavailable. Verify the admin password before revealing the editor, and never store or render the plaintext password in public source or UI.
- The GitHub Pages mirror lives under `/LinkSee/` and uses the existing Sites origin for `/api/team` and `/api/admin/verify`. Keep public assets base-path aware and keep CORS restricted to `https://ranr21094-ai.github.io` so the GitHub-hosted admin continues to persist edits in the same D1 content store.
- Below `700px`, hide the complete event lockup and use `18px` Hero top padding. In the three-card core-feature grid, keep each red number and title on one row, place its description below, and use compact vertical spacing.
- Below `1020px`, prepend an unnumbered `LinkSee` tab to the five member tabs and select it by default. Its solid card summarizes the existing team, project, positioning, event, and contact fields while the portrait stays undimmed. Selecting a member synchronizes that profile with the five-position portrait spotlight; returning to `LinkSee` removes the spotlight.
